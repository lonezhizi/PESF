...
class Dataset_Union_ALL(Dataset):
    def __init__(self, paths, mode='train', data_type='Tr', image_size=128,
                 ...
                 split_num=1, split_idx=0, pcc=False, get_all_meta_info=True):
        self.paths = paths
    ...
    def __getitem__(self, index):
        ...
        if self.mode == "train" and self.data_type == 'Tr':
            ...
            ############
            return subject.image.data.clone().detach().to(torch.long), subject.label.data.clone().detach().to(torch.long),  meta_info
...
if __name__ == "__main__":
    test_dataset = Dataset_Union_ALL(
        paths=[
            '/cpfs01/shared/gmai/medical_preprocessed/3d/iseg/ori_totalseg_two_class/liver/Totalsegmentator_dataset_ct/', ],
        data_type='Ts',
