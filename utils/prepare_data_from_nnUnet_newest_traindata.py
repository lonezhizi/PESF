...
for dataset in dataset_list:
    ...
    for idx, cls_name in meta_info["labels"].items():
        ...
        target_img_dir = osp.join(target_cls_dir, "imagesTr") ####训练集为imagesTr，测试集为imagesTs
        target_gt_dir = osp.join(target_cls_dir, "labelsTr")
        os.makedirs(target_img_dir, exist_ok=True)
        os.makedirs(target_gt_dir, exist_ok=True)
        for item in tqdm(meta_info["training"], desc=f"{dataset_name}-{cls_name}"):     #####这里关键字若是想要转换训练集：training 还是验证集：validation
