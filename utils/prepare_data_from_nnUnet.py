...
for dataset in dataset_list:
    ...
    for idx, cls_name in meta_info["labels"].items():
        ...
        target_img_dir = osp.join(target_cls_dir, "imagesTs") ####训练集为imagesTr，测试集为imagesTs
        target_gt_dir = osp.join(target_cls_dir, "labelsTs")
        os.makedirs(target_img_dir, exist_ok=True)
        os.makedirs(target_gt_dir, exist_ok=True)
        for item in tqdm(meta_info["validation"], desc=f"{dataset_name}-{cls_name}"):    #####这里关键字若是想要转换训练集：training 还是验证集：validation
            ...
            gt_arr = gt_img.get_fdata() #ndarray[768,768,101]
            gt_arr[gt_arr != idx] = 0  ####不是当前标签类的置为0
            gt_arr[gt_arr != 0] = 1 #####!!!!!!!!原始代码，有问题
            ########更改后的。。。如下
            #gt_arr[gt_arr == idx] = 1 #####为当前标签类的置为1
