...
class Dataset_Union_ALL(Dataset): 
    ...
    def __getitem__(self, index):
        ...
        if self.mode == "train" and self.data_type == 'Tr':
            return subject.image.data.clone().detach().to(torch.long), subject.label.data.clone().detach().to(torch.long)
        else:
            return subject.image.data.clone().detach().to(torch.long), subject.label.data.clone().detach().to(torch.long), self.image_paths[index]
    ...
    def _set_file_paths(self, paths):
        ...
        for path in paths:                        ###path:'C:\\pycharm workspace\\SAM-Med3D-main\\data\\train\\adrenal_gland_left\\BTCV'
            d = os.path.join(path, f'labels{self.data_type}')                 ###'C:\\pycharm workspace\\SAM-Med3D-main\\data\\train\\adrenal_gland_left\\BTCV\\labelsTr'
            if os.path.exists(d):
                for name in os.listdir(d):
                    base = os.path.basename(name).split('.nii.gz')[0]
                    label_path = os.path.join(path, f'labels{self.data_type}', f'{base}.nii.gz')            ### 'C:\\pycharm workspace\\SAM-Med3D-main\\data\\train\\adrenal_gland_left\\BTCV\\labelsTr\\img0001.nii.gz'
                    #self.image_paths.append(label_path.replace('labels', 'images'))               ## origin：动态获取输入图像    label_path.replace('labels', 'images')->'C:\\pycharm workspace\\SAM-Med3D-main\\data\\train\\adrenal_gland_left\\BTCV\\imagesTr\\img0001.nii.gz'
                    self.label_paths.append(label_path)
                    ########modified##################################
                    if 'BTCV' in path:
                      traindata_basepath = '/root/autodl-tmp/train-sam3d/data/train_pseudo117cls/adrenal_gland_left/BTCV/imagesTr/'
                      image_path = os.path.join(traindata_basepath, f'{base}.nii.gz')
                      self.image_paths.append(image_path)
                    if 'Amos' in path:
                        traindata_basepath = '/root/autodl-tmp/train-sam3d/data/train_pseudo117cls/adrenal_gland_left/Amos/imagesTr/'
                        image_path = os.path.join(traindata_basepath, f'{base}.nii.gz')
                        self.image_paths.append(image_path)
                    if 'FLARE22' in path:
                        traindata_basepath = '/root/autodl-tmp/train-sam3d/data/train_pseudo117cls/adrenal_gland_left/FLARE22/imagesTr/'
                        image_path = os.path.join(traindata_basepath, f'{base}.nii.gz')
                        self.image_paths.append(image_path)
                    elif 'WORD' in path:
                        traindata_basepath = '/root/autodl-tmp/train-sam3d/data/train_pseudo117cls/adrenal_gland_left/WORD/imagesTr/'
                        image_path = os.path.join(traindata_basepath, f'{base}.nii.gz')
                        self.image_paths.append(image_path)
