...
class MaskDecoder3D(nn.Module):
    ...
    def predict_masks(
        ...
        hs, src = self.transformer(src, pos_src, tokens) ##双头transformer，image_embedding[即：image_embeddings+dense_prompt_embeddings]、image_pe[即图像的位置编码]和point_embedding[即：cat(iou_token、mask_tokens、sparse_prompt_embeddings)]，预测完输出queries（在后续中被作为iou_token和mask_token）, keys（在后续中被作为mask）
        iou_token_out = hs[:, 0, :]
